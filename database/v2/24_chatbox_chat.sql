-- ============================================================================
-- The CHATBOX takes typed messages as well as speech (Roadmap v3, F2).
--
-- A value the CHATBOX read out of a typed sentence is still a model's reading
-- and is checked like a spoken one, so it is counted with them ("voice" in the
-- metrics means "filled by the CHATBOX") and separately as chat_fields.
--
-- The one thing that must not carry over is recording consent: a session that
-- was only typed recorded nothing, and counting it as "voice without consent"
-- would report a breach that did not happen. intake_provenance.voice.audio now
-- says whether anything was recorded; older records, from before the chat
-- existed, were voice by definition.
--
-- A replaced function only. No table changes.
-- ============================================================================
  ...
  SELECT jsonb_build_object(
    ...
    'intake_provenance', (
      SELECT jsonb_build_object(
        ...
        'chat_fields',            COALESCE(sum((intake_provenance->'counts'->>'chat')::int), 0),
        -- Recording consent is about recording. A session that was only typed
        -- sent no audio; one recorded before the chat existed (no "audio"
        -- key) was voice, because nothing else was possible then.
        'voice_without_consent', count(*) FILTER (WHERE intake_mode = 'voice_assisted'
                                   AND COALESCE((intake_provenance->'voice'->>'audio')::boolean, true)
