...
| `qrcode` | `^1.5.4` | The QR code on the referral slip (`reportPdfService.js` → `ackQrPng`), encoding the hospital acknowledgement link. Generated before rendering, because its PNG writer is asynchronous and the render is not; a failure returns null and the slip prints with the typed link only |
