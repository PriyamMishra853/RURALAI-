...
| `RATE_LIMIT_PUBLIC_REFERRAL` | Requests per address per 10 minutes to the hospital acknowledgement link, the one clinical write reachable without signing in. Default 60 |
...
npm run db:migrate 14_baseline_metrics.sql 15_case_referrals.sql 16_referral_tracking.sql 17_intake_provenance.sql 18_follow_ups.sql 19_intake_fields.sql 20_patient_uid.sql 21_patient_consent.sql 22_district_outcomes.sql 23_request_idempotency.sql 24_chatbox_chat.sql 25_model_learning.sql   # before switching on the optional features
npm run rag:seed       # 3 clinical protocols into Qdrant
npm run dataset:export -- --purpose "<why>" --approved-by "<ethics ref>" --out <dir outside the repo>   # verified cases, de-identified, consent-gated
