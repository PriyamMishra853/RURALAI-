...
| `FEATURE_FLAGS` | Comma-separated optional features: `baseline_metrics`, `doctor_referral`, `voice_intake`, `referral_tracking`. Unset means everything is off, which is the demo checkpoint's behaviour. Unknown names are ignored with a warning, never enabled. Apply the feature's migration before switching it on |
